...
while True:
    ...
    if data_type == 6:
        ...
        new_data = Data(data_id=new_id, meter_eui=eui_str, data_battery=battery_s, data_vb=float(vb_s), data_qb=float(qb_s))
    ...
    if data_type == 7:
        ...
        qb_s = str(qb)
        qm_s = str(qm)
        new_data = Data(data_id=new_id, meter_eui=eui_str, data_battery=battery_s, data_vb=float(vb_s), data_vm=float(vm_s), data_p=float(p_s), data_t=float(t_s), data_qb=float(qb_s), data_qm=float(qm_s))
        new_data.data_date = datetime.datetime.now()
        try:
            new_data.save()
        except:
